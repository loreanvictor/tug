...
init() {
  ...
  cat > .tug <<EOF
# .tug — auto-generated by tug init
...
EOF
  ...
  echo -e "▶ Created ${COLOR_PRIM}.tug${NO_COLOR}"
  ...
  echo -e "▶ Created working dir: ${COLOR_PRIM}$APP_DIR${NO_COLOR}"

  # Env dir
  sudo mkdir -p $ETC_DIR
  sudo touch $ENV_FILE
  sudo chmod 600 $ENV_FILE
  echo -e "▶ Created env file: ${COLOR_PRIM}$ENV_FILE${NO_COLOR}"
...
EOF
  ...
  echo ""
  echo -e "${COLOR_SUCC}✔ Initialization complete.${NO_COLOR}"
  echo ""
}
...
build() {
  ...
  echo -e "▶ Build copied to release: ${COLOR_PRIM}$DEST${NO_COLOR}"
  echo -e "${COLOR_SUCC}✔ Build complete.${NO_COLOR}"
}
...
deploy() {
  ...
  echo -e "🚀 ${COLOR_SUCC}Deployment complete.${NO_COLOR}"
}
...
rollback() {
  ...
  if command -v fzf >/dev/null 2>&1; then
    ...
    echo "(fzf not found — using simple menu)"
  ...
  echo -e "${COLOR_SUCC}✔ Rollback complete.${NO_COLOR}"
}
...
cleanup() {
  ...
  echo -e "${COLOR_SUCC}✔ Cleanup complete.${NO_COLOR}"
}
